# go-gin-api
Backend in Go using the Gin framework to generate REST API


## Особенности реализации<br>
1. **GORM ORM** - полная поддержка MySQL с автоматическими миграциями<br>
2. Пул соединений - оптимизированная работа с БД<br>
3. Soft delete - поддержка мягкого удаления<br>
4. Пагинация - эффективная загрузка данных<br>
5. Транзакции - поддержка транзакций для сложных операций<br>
6. Конфигурация через .env - безопасное хранение sensitive данных<br>
7. Docker поддержка - контейнеризация для легкого развертывания<br>
8. Валидация данных - на уровне DTO и БД<br>
9. Уникальные индексы - гарантия целостности данных<br>
10. Логирование запросов - для отладки и мониторинга<br>


## Implementation Features<br>
1. GORM ORM - full MySQL support with automatic migrations<br>
2. Connection Pool - optimized database work<br>
3. Soft Delete - soft delete support<br>
4. Pagination - efficient data loading<br>
5. Transactions - transaction support for complex operations<br>
6. Configuration via .env - secure storage of sensitive data<br>
7. Docker support - containerization for easy deployment<br>
8. Data Validation - at the DTO and DB level<br>
9. Unique Indexes - guarantee data integrity<br>
10. Query Logging - for debugging and monitoring<br><br>

Запуск приложения<br>
bash<br>
go run main.go<br>
